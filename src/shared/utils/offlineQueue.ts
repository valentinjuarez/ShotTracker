  ...
  for (const op of queue) {
    try {
      if (op.type === "CREATE_SESSION") {
        ...
        if (sErr) throw sErr;
        const { error: spErr } = await supabase.from("session_spots").upsert(op.spots);
        if (spErr) throw spErr;
        await deleteLocalSession(op.session.id).catch(() => {});
      } else if (op.type === "UPDATE_SPOT") {
        await supabase.from("session_spots").update({ makes: op.makes }).eq("id", op.spotId);
      } else if (op.type === "FINISH_SESSION") {
        await supabase.from("sessions").update({ status: "DONE", finished_at: op.finishedAt }).eq("id", op.sessionId);
      } else if (op.type === "FINISH_WORKOUT") {
        await supabase.from("workouts").update({ status: "DONE" }).eq("id", op.workoutId);
      }
      processed++;
    } catch {
      failed.push(op);
